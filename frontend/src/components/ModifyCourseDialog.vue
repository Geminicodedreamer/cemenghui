...
export default {
  ...
  watch: {
    ...
    lesson: {
      handler(newVal) {
        if (newVal) {
          this.form = { ...newVal }; // 将选中的课程信息填充到表单中
          if (this.$refs.myQuillEditor) {
            this.$refs.myQuillEditor.quill.root.innerHTML = newVal.lessonIntro;
          }
        } else {
          this.resetForm();
        }
  ...
  methods: {
    ...
    beforeVideoUpload(file) {
      const isVideo = file.type === 'video/mp4' || file.type === 'video/
ChatGPT
avi' || file.type === 'video/mkv';
    ...
    deleteVideo() {
      this.form.video = ''; // 清空 video
    },
    submitForm() {
      if (this.form.lessonName && this.form.lessonAuthor) {
        this.form.lessonIntro = this.$refs.myQuillEditor.quill.root.innerHTML;

        $.ajax({
          url: 'http://127.0.0.1:3000/lesson/modify',
          type: 'POST',
          data: {
